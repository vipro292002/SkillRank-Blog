import Head from 'next/head'
import React from 'react'
import {  ChevronDoubleRightIcon, ChevronLeftIcon, ChevronRightIcon } from '@heroicons/react/24/solid'
import { NextPageWithLayout } from '../../page'
import PrimaryLayout from '../../../components/layouts/primary/PrimaryLayout'
import Link from 'next/link'
import { ICategory } from '../../../lib/types/Category'
import PostCart from '../../../components/cards/post/PostCard'
import MetaHeader from '../../../components/meta'

import { allResources, allSkills, allSQLs, allPowerBIs, allDocuments } from "contentlayer/generated"
import { pick } from '../../../utils/pick'
import { GetStaticPaths, GetStaticProps, GetStaticPropsContext } from 'next'



type Props = {
  resources: any
}

const SkillDetailPage: NextPageWithLayout = ({ posts }: any) => {
  console.log("posts", posts);

  const categories = [
    { id: 1, name: "ReactJS" },
    { id: 2, name: "NextJS" },
    { id: 3, name: "SQL" },
  ]


  return (
    <div>
      <MetaHeader title="Blog" />

      <div>

        <div className="relative bg-gray-50 pt-16 pb-20 px-4 sm:px-6 lg:pt-24 lg:pb-28 lg:px-8">
          <div className="absolute inset-0 bg-white">
            <div className="bg-white h-1/3 sm:h-2/3" />
          </div>

          {posts.map((item: any, index: any) => {
            return <div key={index + 1} className="relative max-w-7xl mb-16 mx-auto">
              <div className="flex justify-between items-end   text-center sm:text-center md:text-left ">
                <div className='basic-3/4'>
                  <h2 className="text-3xl tracking-tight font-extrabold text-gray-900 sm:text-4xl">{item.name}</h2>
                </div >
                <div className='basic-1/4'>
                  <Link href={`/blog/skills/${item.slug}`}>
                    <a className='group float-right text-lg font-bold hover:text-primary-300 ' >
                      Read More
                      <ChevronDoubleRightIcon className='hidden group-hover:inline ml-2 w-5 h-5 ' />
                    </a>
                  </Link>
                </div>
              </div>
              <div className="mt-12 max-w-lg mx-auto grid gap-5 md:grid-cols-2 md:max-w-none lg:grid-cols-3 lg:max-w-none">
                {item.value.map((post: any, index: any) => {
                  if (post.orderIndex === 1) {
                    return <PostCart key={index + 1} {...post} />
                  }
                })}
                {item.value.map((post: any, index: any) => {
                  if (post.orderIndex !== 1) {
                    return <PostCart key={index + 1} {...post} />
                  }
                })}
              </div>
            </div>
          })}



        </div>




      </div>


    </div>
  )
}

export default SkillDetailPage

SkillDetailPage.getLayout = (page) => {
  return <PrimaryLayout>{page}</PrimaryLayout>;
};

// export const getStaticPaths: GetStaticPaths = async () => {
//   const paths = [
//     // /
//     { params: { filter: [] } },
//     // /videos
//     { params: { filter: ["sql"] } },
//     // /blog
//     { params: { filter: ["powerbi"] } },

   
//   ]
//   return {
//     paths,
//     fallback: false,
//   }
// };

export const getStaticPaths: GetStaticPaths = async () => {
  return {
      paths: allResources.map((resource) => ({
          params: { skill_slug: resource.slug },
      })),
      fallback: false,
  };
};

export const getStaticProps: GetStaticProps<any> = async (context: GetStaticPropsContext) => {
  console.log("context", context);

  const categories = [
    { id: 1, name: "ReactJS" },
    { id: 2, name: "NextJS" },
    { id: 3, name: "SQL" },
  ]

  const temp1: any = allPowerBIs
    .map((resource: any) =>
      pick(resource, [
        'slug',
        'title',
        'description',
        'publishedAt',
        'readingTime',
        'author',
        'category',
        'image',
        "tags",
        "orderIndex"
      ])
    )
    .sort(
      (a: any, b: any) =>
        Number(new Date(b.publishedAt)) - Number(new Date(a.publishedAt))
    );

  const temp2: any = allSQLs
    .map((resource: any) =>
      pick(resource, [
        'slug',
        'title',
        'description',
        'publishedAt',
        'readingTime',
        'author',
        'category',
        'image',
        "tags",
        "orderIndex"
      ])
    )
    .sort(
      (a: any, b: any) =>
        Number(new Date(b.publishedAt)) - Number(new Date(a.publishedAt))
    );

  const temp = [...allSkills, ...allSQLs, ...allPowerBIs]
  const posts: any = [
    {
      name: "PowerBI", slug: "powerbi", value: temp1.sort(
        (a: any, b: any) =>
          Number(new Date(b.publishedAt)) - Number(new Date(a.publishedAt))
      )
    },
    { name: "SQL", slug: "sql", value: temp2 },
  ]


  return { props: { posts } };
};

