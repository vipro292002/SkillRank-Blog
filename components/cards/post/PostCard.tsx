...
const PostCart: React.FC<IPostCard> = ({ title, slug, category, description, publishedAt, image, readingTime, author, tags }: IPostCard) => {
  return (
    <div className="group flex flex-col rounded-lg shadow-lg overflow-hidden cursor-pointer hover:-translate-y-1 hover:scale-105 transition duration-500 ease-in-out" >
      ...
      <div className="flex-1 bg-white dark:bg-dark-300 p-6 flex flex-col justify-between">
        <div className="flex-1">
          ...
          <Link href={`/blog/category/${category.slug}`}>
            <span className="text-sm font-medium text-primary-100 hover:underline hover:text-primary-300">
              {category.name}
            </span>
          </Link>
        ...
        <div className="mt-6 flex items-center gap-2 ">
          {tags.map((item: any, index: number) => {
            ...
            >
              ...
              <Link href={`/blog/category/tag/${item.slug}`} >
