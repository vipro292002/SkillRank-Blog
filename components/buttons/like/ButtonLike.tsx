  ...
  return (
    <div className="flex items-center space-x-2">
      <div className="relative">
        ...
        <button
          className={cx(
            " shadow-lgx group relative block transform overflow-hidden rounded-lg bg-gradient-to-tl from-white/5 to-white/30 p-1 transition-all duration-300 ease-out hover:scale-[1.2] hover:rounded-[10px] active:scale-100 active:rounded-lg focus:outline-none focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-rose-500/70",
            {
        ...
        >
          <div
            className={cx(
              "absolute inset-0 transform-gpu bg-gradient-to-tl from-primary-100 to-primary-400 transition-transform",
              {
          ...
          <HandThumbUpIcon className="relative w-5 transform text-rose-100 transition delay-100 duration-500 ease-out group-hover:scale-110" />
        </button>
      </div>

      {/* Like counter text */}
      <div className="text-lg font-medium leading-none text-rose-100/90">
        {isLoading ? <span>Loading...</span> : <span>{likes}</span>}
      </div>
    </div>
  )
}
